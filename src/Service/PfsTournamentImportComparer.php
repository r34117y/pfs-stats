...
final class PfsTournamentImportComparer
{
    ...
    private function compareTournamentRow(PfsTournamentImportPlan $plan, array &$findings): void
    {
        $actual = $this->connection->fetchAssociative(
            'SELECT id, dt, name, fullname, winner, trank, players, rounds, team, mcategory, wksum, sertour, start, referee, place, organizer, urlid
            FROM PFSTOURS WHERE id = :id',
    ...
    private function compareTournamentResults(PfsTournamentImportPlan $plan, array &$findings): void
    {
        $actualRows = $this->connection->fetchAllAssociative(
            'SELECT player, place, gwin, glost, gdraw, games, trank, brank, points, pointo, hostgames, hostwin, masters
            FROM PFSTOURWYN WHERE turniej = :id',
            ['id' => $plan->tournament->id],
        );
    ...
    {
        $actualRows = $this->connection->fetchAllAssociative(
            'SELECT runda, stol, player1, player2, result1, result2, ranko, host
            FROM PFSTOURHH WHERE turniej = :id',
            ['id' => $plan->tournament->id],
        );
